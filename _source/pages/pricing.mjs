...
`;
    return {
      title: 'Сколько стоит умный дом под ключ: калькулятор цены · Smart Home',
      description: 'Калькулятор стоимости умного дома под ключ: квартира от 690 000 ₽, загородный дом с баней и участком от 1,84 млн ₽, офис по площади. Ориентиры в рублях с НДС.',
